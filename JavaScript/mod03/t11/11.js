...
function modalPicture(evt) {
  const dialog = document.querySelector("dialog");
  const imgTarget = document.querySelector('dialog > img');
  const span = document.querySelector('dialog > span');
  for (let i = 0; i < allArticles.length; i++) {
    if (evt.currentTarget.className === allArticles[i].className) {
      imgTarget.src = picArray[i]['image']['large'];
      imgTarget.alt = picArray[i]['title'];
      dialog.showModal();
...
for (let info of picArray) {
  ...
  let target2 = document.querySelector(".card:last-child");
  target2.innerHTML = `<h2>${info['title']}</h2>
                       <figure>
                       <img src=${info['image']['medium']} alt=${info['title']}>
...
/* Constructing the dialog/span/img*/
const targetBody = document.querySelector("body");
let newDialog = document.createElement('dialog');
let newSpan = document.createElement('span');
newSpan.appendChild(document.createTextNode('\u2715'));
newDialog.appendChild(newSpan);
let newImg = document.createElement('img');
newDialog.appendChild(newImg);
targetBody.appendChild(newDialog);

/* Adding eventlisteners for all articles and adding unique classes */
let allArticles = document.querySelectorAll('article');
allArticles.forEach(item => {item.addEventListener('click', modalPicture)});
for (let i = 0; i < allArticles.length; i++) {
  allArticles[i].classList.remove("card")
  allArticles[i].classList.add(`card${[i]}`)
}
